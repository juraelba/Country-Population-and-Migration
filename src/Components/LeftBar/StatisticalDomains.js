...
const mapStateToProps = state => ({
    statistical_domains : state.selections.statistical_domains,
    utils : state.utils
});
...
class StatisticalDomains extends Component{
    ...
    getOptions(props = this.props) {
        return Object.values(props.statistical_domains.options);
    }

    handleOnChange = (selected_id) => {
        const { selectionType, statistical_domains } = this.props;
        const selection_type = selectionType;
        const { selected_ids, options } = statistical_domains;
    ...
    render(){
        const { heading, statistical_domains } = this.props;
        const { show, searchValue, searchedResults, isLoading } = this.state;
        const options = this.getOptions();
        const selection_ids = statistical_domains["selected_ids"];
        ...
        return (
            <Container>
                ...
                <Row isShow={show}>
                <SearchBar placeholder={`Filter ${heading}`} value={searchValue} onChange={this.handleSearch}/>
                <SortButton />
                {
                    isLoading ?
                        (
                        ...
                        :
                        (
                        <OptionsContainer className="statistical-domains">
                            <Options>
                                {
...
export default connect(mapStateToProps, mapDispatchToProps)(StatisticalDomains);
