...
const mapStateToProps = state => ({
    datasets : state.selections.datasets,
    utils : state.utils
});
...
const SortButton = styled.span`
    ...
    transform: ${props => props.isSort ? "none" : "scaleY(-1)" };
    margin-left: 10px;
    display: inline;
`;
...
class DataSets extends Component{
    ...
    getOptions(props = this.props) {
        return Object.values(props.datasets.options);
    }

    handleOnChange = (selected_id) => {
        const { selectionType, datasets } = this.props;
        const selection_type = selectionType;
        const { selected_ids, options } = datasets;
    ...
    handleSort = () => {
        ...
        if(sort){
            searchedResults.sort((a,b) => (b.names[0]["value"] > a.names[0]["value"]) ? 1 : ( (a.names[0]["value"] > b.names[0]["value"]) ? -1 : 0));
        }else{
            searchedResults.sort((a,b) => (a.names[0]["value"] > b.names[0]["value"]) ? 1 : ((b.names[0]["value"] > a.names[0]["value"]) ? -1 : 0));
        }
    ...
    render(){
        const { heading, datasets } = this.props;
        const { show, searchValue, sort } = this.state;
        const options = this.getOptions();
        const isLoading = options === null;
        const selection_ids = datasets["selected_ids"];
                ...
                <SortButton isSort={sort} onClick={this.handleSort}/>
                {
                    isLoading ?
                        ...
                        <OptionsContainer className="datasets">
                            <Options>
                                {
                                    options.map((searchedResult, index)=>{
